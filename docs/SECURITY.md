...
- **Fixed literal, never interpolated.** The appended block resolves via the
  ...
  naming the four §23 shortcuts, authored with the same discipline as the
  member/admin capability text blocks (`communityInfoMemberCapabilities`/
  `communityInfoAdminCapabilities`, §26/§28) — no caller or message data ever
  reaches any of them. See §27 for the `'mi'`-preference behaviour.
...
- **Scoped to the member-tier segment only, at the time.** The admin/
  ...
  WhatsApp shortcuts half of that gap (issue #1034); §28 closes the
  admin/super-admin half (issue #1056).
...
### 28. `community_info` extends the `'mi'` preference to the admin/super-admin segments (issue #1056)
...
- **The WhatsApp `!`-shortcuts block (§24) is unaffected by this issue** — out
  of its scope; it got its own `mi` variant separately, see §27 (issue #1034).
