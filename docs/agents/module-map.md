...
- `src/module/agent/tools/linkSummary.ts` — 🔒 The member-tier `summarize_link` ToolDef domain (off by default, `LINK_SUMMARY_ENABLED`): opens only a URL that appears in a recent inbound, person-authored message in the caller's own conversation, fetching the POSTED form over agent-base's `safeFetch` with that URL's host as the entire allowlist — so the model selects a posted link but never composes one — and returns the page quarantined via `untrusted()`, behind a per-caller daily reserver and dedup window.
