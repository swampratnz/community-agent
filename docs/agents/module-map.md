...
- `src/module/accessRequestStaleAlert.ts` — Guild-wide crossing-latch alert (issue #1100) DMing every admin once when the count of pending access requests older than 168h first leaves zero — the push complement to `list_access_requests`' pull-only view and the weekly `oldestAccessRequestAgeDays` digest line, and the fifth and last of `review_queue`'s five queues to get one. Mirrors `appealStaleAlert.ts`'s pattern and reuses its `alertAdmins` helper unchanged. The same tick also sends the GUEST who filed the request a one-time "still being reviewed" DM (issue #1421, `notifyAccessRequestStale` in `notify.ts`), independent of the admin latch and idempotent via `access_request_stale_notices` (`storage/accessRequestStaleNotices.ts`) — UNLIKE its `88`-`91` id-only siblings this table carries requester identity (access requests have no anonymous numeric id) and so registers a `registerPurgeContributor` hook, plus a per-tick `pruneAccessRequestStaleNotices` reconciliation against the full pending scan.
...
- `src/module/agent/tools/accessAndSuggestions.ts` — The `list_access_requests`/`decline_access_request`/`list_suggestions`/`resolve_suggestion` ToolDef domain: guest access-request and member-suggestion triage.
...
- `src/module/agent/tools/fetchPage.ts` — 🔒 The admin-tier `fetch_page` ToolDef domain: the bot's only caller-driven-URL egress, over agent-base's `safeFetch`, gated by the operator host allowlist plus a per-caller daily reserver and a per-caller-per-URL dedup window, returning the page quarantined via `untrusted()`. `check_knowledge_source` (`knowledgeAdmin.ts`) is a second, narrower egress path — its target is never caller-supplied at call time, only an already-admin-authored `sourceUrl` looked up by id.
...
- `src/module/agent/tools/knowledgeAdmin.ts` — The admin knowledge-curation ToolDef domain: save/list/update/delete/merge knowledge, duplicate/conflict/gap audits, the candidate review queue, answer-feedback roll-ups, and an on-demand single-entry link-rot re-check (`check_knowledge_source`, reusing `context/linkCheck.ts`'s SSRF-hardened probe).
- `src/module/agent/tools/knowledgeMember.ts` — The member-facing knowledge ToolDef domain: `knowledge_search` (with its turn-state gap/stale correlation writes), `knowledge_for_me` (searches knowledge using the caller's own published interests as the query), `list_knowledge_topics`, `most_helpful_knowledge`, `suggest_knowledge`, `withdraw_knowledge_tip`.
- `src/module/agent/tools/linkSummary.ts` — 🔒 The member-tier `summarize_link` ToolDef domain (off by default, `LINK_SUMMARY_ENABLED`): opens only a URL that appears in a recent inbound, person-authored message in the caller's own conversation, fetching the POSTED form over agent-base's `safeFetch` with that URL's host as the entire allowlist — so the model selects a posted link but never composes one — and returns the page's READABLE TEXT (not raw HTML) quarantined via `untrustedWeb()`, behind a per-caller daily reserver and dedup window.
...
- `src/module/agent/tools/social.ts` — The member-discovery ToolDef domain: interests (`set_my_interests`/`who_is_into`/`set_interest_match_alerts`), peer help (`set_helper_availability`/`find_helper`), and the project showcase (`share_project`/`list_projects`/`request_project_connection`).
...
- `src/module/agent/tools/webResearch.ts` — 🔒 The member-tier `web_research` ToolDef domain (off by default, `WEB_RESEARCH_ENABLED`): runs one ISOLATED `query()` carrying only the question — no history, no module tools, exactly `WebSearch`, never `WebFetch` — gated on `turnState.knowledgeSearchMissed` so `knowledge_search` always runs first; returns the answer quarantined with https-only sources and records the spend as the `web_research` background job.
- `src/module/agentModule.ts` — 🔒 THE module manifest: every extension point this deployment fills (notices, tool tiers/server parts/flag predicates, skills, prompt sections, commands, bad words, personas, turn-state finalizer, policy keys, runtime secrets, migrations) as one inspectable object, handed to agent-base's `createAgent` by `src/index.ts`. Its `init()` asserts the NZ display settings rather than letting a missing env var silently re-render every time in UTC.
- `src/module/appealStaleAlert.ts` — Guild-wide crossing-latch alert (issue #1020) DMing every admin once when the count of open moderation appeals older than 72h first leaves zero — the push complement to `list_appeals`' pull-only view and the weekly `oldestOpenAppealAgeDays` digest line. The same tick also sends the APPELLANT a one-time "still being reviewed" DM per stale, non-withdrawn appeal (issue #1413, `notifyAppealStale` in `notify.ts`), independent of the admin latch and idempotent via `appeal_appellant_stale_notices` (`storage/appealAppellantStaleNotices.ts`).
...
- `src/module/commands.ts` — The community command list (plan §3 `commands` row): the ordered `{name, platforms, handler}` entries (kb, projects, whois, guidelines, digest, status, warnings, mysubmissions, mydata, events, help), registered by the manifest into the package's command registry; WhatsApp handlers live here verbatim, Discord halves are bound by slashCommands.ts.
...
- `src/module/crossingLatch.ts` — `persistedCrossingLatch` (issue #1198): wraps agent-base's pure `initialUsageAlertTracker`/`stepUsageAlertTracker` guild-wide crossing latch with `readPolicy`/`updatePolicy` persistence, so the six stale-backlog admin alerts below survive a process restart instead of re-firing a duplicate DM. Shared verbatim by all six — not six independent copies.
...
- `src/module/interestMatchAlert.ts` — Per-member crossing-latch alert (issue #1332) DMing an opted-in member once when their own `who_is_into` self-match count first leaves zero — the push complement to `who_is_into`'s pull-only self-match path, structurally mirroring `reportStaleAlert.ts`'s per-key shape but scanning the `interest_match_alert_optins` opt-in table instead of `listAdmins()`.
...
- `src/module/knowledgeCandidateStaleAlert.ts` — Guild-wide crossing-latch alert (issue #1073) DMing every admin once when the count of pending knowledge candidates older than 168h first leaves zero — the push complement to `list_knowledge_candidates`' pull-only view and the weekly `oldestPendingCandidateAgeDays` digest line. Mirrors `appealStaleAlert.ts`'s pattern and reuses its `alertAdmins` helper unchanged. The same tick also sends a member-sourced candidate's SUBMITTER a one-time "still being reviewed" DM (issue #1408, `notifyKnowledgeCandidateStale` in `notify.ts`), independent of the admin latch and idempotent via `knowledge_candidate_stale_notices` (`storage/knowledgeCandidateStaleNotices.ts`).
...
- `src/module/reportStaleAlert.ts` — Per-admin crossing-latch alert (issue #1084) DMing an admin once when the count of open content reports in THEIR OWN `conversationsForUser` scope older than 48h first leaves zero — unlike its guild-wide siblings (`appealStaleAlert.ts`, `knowledgeCandidateStaleAlert.ts`), `content_reports` is conversation-scoped per admin with an accused-admin exclusion, so the latch and DM are per admin, not guild-wide. The same tick also sends the REPORTER a one-time "still being reviewed" DM per stale report (issue #1375, `notifyReportStale` in `notify.ts`), independent of the admin latch and idempotent via `report_reporter_stale_notices` (`storage/reportReporterStaleNotices.ts`).
- `src/module/rosterStaleAlert.ts` — Guild-wide crossing-latch alert (issue #1136), combined across every `'gated'`-access-mode platform, DMing every admin once when the count of `not_members` (onboarding queue) rows older than 168h first leaves zero — the push complement to `list_roster` (filter: not_members)'s pull-only view and the weekly `notMembersCount` digest line, and the sixth and last of the admin review queues to get one. Mirrors `appealStaleAlert.ts`'s pattern and reuses its `alertAdmins` helper unchanged; reuses `adminDigest.ts`'s exact `'gated'`-mode condition so an `'open'`-mode platform, where a not_members row already has full access, is never nagged.
...
- `src/module/storage/` — The community-owned policy keys (guidelines and welcome message, plus their te reo Māori variants) with their typed accessors, this module's schema fragments (`schema/`), and the module-owned auxiliary tables beside base tables (`accessRequestResolutions.ts`, `suggestionWithdrawals.ts`, `appealWithdrawals.ts`, `findHelperRequests.ts`, `projectNoteRecords.ts`, `interestMatchAlertOptIns.ts`, `humanHelpRequestLog.ts`, `reportReporterStaleNotices.ts`, `knowledgeCandidateStaleNotices.ts`, `appealAppellantStaleNotices.ts`, `suggestionSubmitterStaleNotices.ts`, `accessRequestStaleNotices.ts`), all contributed through the manifest.
- `src/module/storage/schema/` — 🔒 This deployment's schema contribution (plan §3 `migrations`): the fragments agent-base does NOT ship, contributed through `AgentModule.migrations` and concatenated AFTER every base fragment as one atomic query. The 80-band fragments: standing-preference value allowlists, and the module-owned auxiliary tables (access-request resolutions, suggestion/appeal withdrawals, find-helper requests, `85-project-note-records.sql`'s project-note authorship + withdrawal pair, `88`–`91`'s id-only submitter/appellant/reporter stale-notice tables, `92-access-request-stale-notices.sql`'s identity-bearing, purge-hooked stale-notice table). Never re-declare a base fragment, and never reshape a constraint base owns — pinned by `tests/schemaConstraintIdempotency.test.ts`.
...
- `src/module/suggestionStaleAlert.ts` — Guild-wide crossing-latch alert (issue #1091) DMing every admin once when the count of pending (`status = 'new'`) member suggestions older than 168h first leaves zero — the push complement to `list_suggestions`' pull-only view and the weekly `oldestPendingSuggestionAgeDays` digest line. Mirrors `appealStaleAlert.ts`'s pattern and reuses its `alertAdmins` helper unchanged. The same tick also sends the SUBMITTER a one-time "still being reviewed" DM per stale, non-withdrawn suggestion (issue #1415, `notifySuggestionStale` in `notify.ts`), independent of the admin latch and idempotent via `suggestion_submitter_stale_notices` (`storage/suggestionSubmitterStaleNotices.ts`) — the fourth and last member-contribution queue to get this treatment.
