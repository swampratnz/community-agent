...
- `src/module/agent/tools/projectNotes.ts` — The `project_recall`/`project_note`/`project_list`/`withdraw_project_note`/`my_project_notes` ToolDef domain: team-project shared memory, access-scoped in SQL via `visibleProjectIds`.
