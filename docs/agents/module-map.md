...
- `src/base/commands/registry.ts` — The one file in `src/base/commands/` today: the once-per-process `registerCommands` slot, `registeredCommands()` (which THROWS rather than reporting an empty roster, so a forgotten side-effect import can't silently stop every command matching), and `bindDiscordCommand`'s late binding.
...
- `src/base/media/` — Local-only media handling that carries no community content: the one Whisper voice-transcription module all three adapters share, decoded on-host and never shipped to a third party. Base despite the community-sounding feature — it is mechanism with no community imports.
