...
- `src/module/agent/tools/fetchPage.ts` — 🔒 The admin-tier `fetch_page` ToolDef domain: the bot's only caller-driven egress, over agent-base's `safeFetch`, gated by the operator host allowlist plus a per-caller daily reserver, returning the page quarantined via `untrusted()`.
