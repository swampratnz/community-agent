...
- `src/module/agent/tools/fetchPage.ts` — 🔒 The admin-tier `fetch_page` ToolDef domain: the bot's only caller-driven-URL egress, over agent-base's `safeFetch`, gated by the operator host allowlist plus a per-caller daily reserver, returning the page quarantined via `untrusted()`. `check_knowledge_source` (`knowledgeAdmin.ts`) is a second, narrower egress path — its target is never caller-supplied at call time, only an already-admin-authored `sourceUrl` looked up by id.
