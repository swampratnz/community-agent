...
- `src/module/connectionOutcomeFollowup.ts` — Requester-only daily follow-up (issue #1354) DMing the caller of a real `find_helper`/`request_project_connection` handoff, a fixed window later, asking whether it helped — the push complement to `rate_connection_outcome`'s pull-only rating tool, and the missing OUTCOME signal alongside `helperMatchesCount`/`projectConnectionsCount`'s existing ACTIVITY counts (adminDigest.ts). Never contacts the matched helper/project owner; idempotent via `connectionOutcomes.ts`'s atomic claim-and-stamp, not the job's own tick cadence.
...
- `src/module/storage/` — The community-owned policy keys (guidelines and welcome message, plus their te reo Māori variants) with their typed accessors, this module's schema fragments (`schema/`), and the module-owned auxiliary tables beside base tables (`accessRequestResolutions.ts`, `suggestionWithdrawals.ts`, `appealWithdrawals.ts`, `findHelperRequests.ts`, `projectNoteRecords.ts`, `connectionOutcomes.ts`), all contributed through the manifest.
