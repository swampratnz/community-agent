...
- `src/agent/featureFlags.ts` — Base feature-flag predicate slot: core.ts's subtractive per-turn filter reads `flaggedToolPredicates()` from here, registered by the tool registry at import time (fail-loud before registration).
...
- `src/agent/systemPrompt.ts` — 🔒 The slot assembler for the system prompt: frozen top-level slot order over `promptSpine.ts`'s security spine, registered community sections (including the style/language slot bodies), persona voice and role/policy notes — no community prose lives here. Byte-stability per (role, policy, persona, day) is pinned by `tests/systemPromptByteStability.test.ts`.
- `src/agent/toolServer.ts` — 🔒 The base tool-hosting kernel: `buildToolServer` composes the per-turn MCP server from the module-registered parts (`registerToolServerParts` — server name, registry, context factory), failing closed until the tool registry has been imported.
- `src/agent/tools.ts` — The barrel for the tool-registry split: re-exports every moved helper/notify/registry symbol (plus `buildToolServer` from the base kernel) and loads the registry's module-scope registrations. Find your tool by name in `src/agent/tools/` before reading anything else.
...
- `src/auth/` — 🔒 Identity and role resolution: tiers come from env plus the `community_users` table, never from message content. Four files, all small and worth reading in full — `tiers.ts` is the dependency-free tier lattice (`atLeast`/`assertAtLeast`, imported by the tool domain files); `rbac.ts`'s tier lists are REGISTERED by the tool registry (`registerToolTiers`, called at src/agent/tools/index.ts module scope) and `toolsForRole` fails closed until that registration has run; `memberId.ts` is a thin, fail-closed dispatcher over the platform registry's per-adapter member-id rules (`src/platforms/*/memberIdRules.ts`).
...
- `src/backgroundJobs.ts` — The knowledge/context/status/dev-team job run functions and their `JobSpec` entries; the shared tracked-job wrapper they start through lives in `src/jobs/trackedJob.ts`.
...
- `src/commands.ts` — The community command list (plan §3 `commands` row): the ordered `{name, platforms, handler}` entries (kb, projects, whois, guidelines, digest) plus `WhatsAppTextCommandDeps`, self-registered into `commands/registry.ts` at module scope; WhatsApp handlers live here verbatim, Discord halves are bound by slashCommands.ts.
...
- `src/commands/registry.ts` — The one file in `src/commands/` today — see the subsystem entry above.
...
- `src/moderation/badWords.ts` — The community-owned default bad-word list, registered into `moderation/wordlist.ts`'s fail-loud slot at import time; operators still extend it with MODERATION_BAD_WORDS.
...
- `src/platforms/textPacks.ts` — The community-owned adapter text packs (plan §3 `textPacks`): the join-welcome and manual `warn_user` DM strings for Discord, Baileys and WhatsApp Cloud, injected by `factories.ts` into each adapter's now-required text-pack constructor parameter.
...
- `src/routerWiring.ts` — The router's production wiring: `makeRouterDeps`, the ONE place the real implementation behind every `RouterDeps` field is named, so the router mechanism itself never imports the community content its defaults point at; `index.ts` (and every router test) composes `new Router(makeRouterDeps(...))`.
...
- `src/storage/policyStore.ts` — The runtime-policy mechanism: 30s-TTL cached reads/writes over the `policies` table, the base keys (`code_answers`, `paused`) with their accessors, and `registerPolicyKeys` for module keys — the community keys + typed accessors live in `policies.ts`, which registers at its own import time. Reads/writes of an unregistered key THROW (fail loud), and `resetPolicyCacheForTests` lives here.
...
- `src/strings/` — The locale/style notice catalogue (agent-base plan item 6): `catalogue.ts` is the base mechanism — `notice(id, {language, style})` reads the single pack `notices.ts` registers at its own import time (`registerNoticePack`; unregistered reads THROW, never blank text) and owns the "'mi' beats 'plain', default English" precedence every call site used to re-encode — and `notices.ts` is the community pack holding every notice's English/mi/plain text verbatim, plus the `NoticeIdMap` augmentation that keeps per-id return types. Base consumers import `notice` from `catalogue.ts`; `src/index.ts` imports the pack first. The old `*_MI`/`*_PLAIN` constants remain importable from their original modules as derived consts. The DB-facing preference unions and set_* tool enums stay CLOSED on purpose.
