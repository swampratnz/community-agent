...
| `web_research` (member web research as an ISOLATED sub-turn: one separate `query()` whose prompt is only the question — no history, no module tools, `settingSources: []`, exactly `WebSearch`, never `WebFetch`. Refuses unless a `knowledge_search` earlier in the same turn found nothing relevant (`turnState.knowledgeSearchMissed`). The answer returns quarantined via `untrusted()` with https-only sources. Per-caller daily cap `WEB_RESEARCH_DAILY_LIMIT` plus a dedup window, turn ceiling `WEB_RESEARCH_MAX_TURNS`, a wall-clock timeout; spend recorded as the `web_research` background job. Off by default: `WEB_RESEARCH_ENABLED`) | ❌ | ✅ | ✅ | ✅ |
