...
6. **`community_info` names every tool the caller actually has.** The
   ...
   admin segment). The WhatsApp shortcuts block itself got the same treatment
   (issue #1034): it resolves via the module notice pack's
   `whatsappTextCommands` entry, selected on the same already-read `language`
   value, so a `'mi'`-preference WhatsApp member no longer sees an abrupt
   English flip mid-reply.
