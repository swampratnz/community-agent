...
| `set_my_interests` (publish self-declared interests for member-to-member discovery; `text | 'clear'`, one row per identity, upsert/clear semantics; explicitly floors at `member`, excluding open-mode guests) / `who_is_into` (embedding-similarity search over published interests only; same `member` floor; a caller with no published interests of their own can still search) | ❌ | ✅ | ✅ | ✅ |
