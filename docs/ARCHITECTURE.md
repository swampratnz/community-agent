...
`src/module/agent/tools.ts`) → MIME allowlist (`image/png`, `image/jpeg`,
...
4. Admins can promote durable facts into `knowledge` via `save_knowledge`, and
   ...
   patterns worth turning into a knowledge entry. `src/module/adminDigest.ts` (issue
   ...
   *themes* — the "thumbs-down themes shrinking in the digests" other half of
   ...
   (the same guild-wide aggregate `src/module/memberDigest.ts` already runs weekly for
   ...
   and, via issue #801, the pending-knowledge-candidates line
   (`oldestPendingCandidateAgeDays()`, rendered independently of the
   knob-gated stale-days sub-count), each a `MIN(created_at)` (or
   `MIN(first_requested_at)`) aggregate over exactly the same scoped row set
   its sibling `COUNT(*)` already reads — so the report age inherits
   `countOpenReports`'s conversation/DM scoping and the appeal age inherits
   `countOpenAppeals`'s platform scoping, and an admin can never see the age
   of a row outside their scope. Each age only ever decorates its
   already-nonzero count line and only when non-null (an empty scoped set has
   no meaningful age), so a quiet week is byte-identical to before. The DM
   sends when *any* of the eleven signals is non-zero, and
...
query), rendering one line per queue: every one of the five lines shows the
oldest item's age in whole days once that queue is non-empty (omitted when
empty, matching each `oldest*AgeDays`'s own `null`-when-empty contract) —
knowledge candidates gained this via `oldestPendingCandidateAgeDays()` in
issue #801, closing the one gap #743/#787 each named and deferred. Reports
use `callerScope()` plus the same linked-identity `viewerUserIds` exclusion
`list_reports` computes, never a guild-wide count; appeals use the caller's
own platform (both the count and, via `oldestOpenAppealAgeDays(caller.platform)`,
issue #787, the age), matching `list_appeals`. Access requests, suggestions,
...
fixed, hand-maintained `FEATURE_FLAG_MAP` (`src/module/agent/tools.ts`) allowlists
...
`src/module/agent/skills/agent-architecture-review/SKILL.md` and shares this exact
...
- **admin** — granted by a super admin (`grant_admin`); stored in
  ...
  notice and decide whether to revoke. `src/module/departedAdminAlert.ts` (off
...
| `knowledge_for_me` (search the knowledge base using the caller's OWN published interests, set via `set_my_interests`, as the query — composes `getPublishedInterestsForOwners` (the same self-scoped lookup `who_is_into({mine:true})` uses), `searchKnowledge`, and the existing `formatKnowledgeSearchResults` verbatim, no new relevance threshold/caveat/citation code; with no published interests, returns the same "publish interests first" guidance and makes no search call — issue #1287) | ❌ | ✅ | ✅ | ✅ |
...
| `web_research` (member web research as an ISOLATED sub-turn: one separate `query()` whose prompt is only the question — no history, no module tools, `settingSources: []`, exactly `WebSearch`, never `WebFetch`. Refuses unless a `knowledge_search` earlier in the same turn found nothing relevant (`turnState.knowledgeSearchMissed`) AND its own pre-check finds no curated hit for the exact question it was given. The answer returns quarantined via `untrusted()` with https-only sources. Per-caller daily cap `WEB_RESEARCH_DAILY_LIMIT` plus a dedup window, turn ceiling `WEB_RESEARCH_MAX_TURNS`, a wall-clock timeout; spend recorded as the `web_research` background job. Off by default: `WEB_RESEARCH_ENABLED`) | ❌ | ✅ | ✅ | ✅ |
| `summarize_link` (read a page a PERSON posted in the caller's own conversation within `LINK_SUMMARY_LOOKBACK_HOURS`: the URL must appear in a recent inbound message — the bot's own messages never count — and the posted form is what is fetched, over `safeFetch` with that URL's host as the entire allowlist, so the model can select a posted link but never compose one and a cross-host redirect is refused. Returned quarantined via `untrusted()`; per-caller daily cap `LINK_SUMMARY_DAILY_LIMIT` plus a dedup window. Off by default: `LINK_SUMMARY_ENABLED`) | ❌ | ✅ | ✅ | ✅ |
...
`warn_user` action (`src/module/agent/tools.ts`) — gets the same treatment (issue
...
`notifySuggestionResolved`/`notifyReportResolved` in `src/module/agent/tools.ts`,
...
2. **Pending-access queue**. When a gated guest addresses the bot,
   ...
   `GATED_NOTICE_MI` gets the same treatment via its te reo sibling
   `appendWaitClauseMi` (issue #716; before that, it was left byte-for-byte
   unchanged as a documented follow-up). A brand-new guest (`< 1` day) still
   renders byte-identical to before, in either language. The wording is
   deliberately neutral ("on record") rather than claiming an admin was
   actively notified, since the real-time alert below is flag-gated and may
   not have fired for this request. The clause interpolates only a plain
   integer day count, never a name or message content, so it needs no
   `sanitizeName`-style treatment. Threading `firstRequestedAt` into the
   notice means the alert-disabled fire-and-forget upsert (issue #480, below)
   is now awaited on both branches that actually render a static notice — an
...
`src/module/context/builder.ts` (issue #51) is the learning step on top of storage:
...
`src/module/context/export.ts` regenerates its copy at `CONTEXT_EXPORT_PATH` —
...
`knowledge_search`/`catch_up`. `src/module/memberDigest.ts` (issue #645) widens the
...
- **`community_digest`** (member-tier chat tool, `src/module/agent/tools.ts`):
...
`startStatusCheck` (`src/module/backgroundJobs.ts`) already DMs super admins when
...
project_unarchive ──▶ restores exactly the access project_archive revoked
...
Scope was originally narrow (issue #744's guardrail): exactly these four
commands, no WhatsApp equivalent, and no `shortcut_hits` tracking of
slash-command usage — the latter two named as deferred growth paths, not
that feature. Both have since been built. WhatsApp has no native
command-picker UI to register these against, so it instead gets a
...
- **`jobs` field on `/healthz`** (issue #467) — every opt-in background job
  ...
  `src/module/jobs/registry.ts` — `BackgroundJobName` is an open string type, never
...
`src/module/usageCostDigest.ts` (off unless `USAGE_COST_DIGEST_ENABLED`, issue #578)
...
`src/module/departedAdminAlert.ts` (off unless `DEPARTED_ADMIN_ALERT_ENABLED`, issue
...
`src/module/engagementAlert.ts` (off unless `ENGAGEMENT_ALERT_ENABLED`, issue #568)
...
`src/module/adminLeverageAlert.ts` (off unless `ADMIN_LEVERAGE_ALERT_ENABLED`, issue
