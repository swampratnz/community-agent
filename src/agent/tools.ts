 ...
 * With `staleDays <= 0` (the default), `isKnowledgeStale` always returns
 * `false`, so every pair is "both fresh" and this is a no-op — output stays
 * byte-identical to pre-#308 behaviour for any deployment that hasn't opted
 * into staleness tracking.
 */
  ...
  return ordered
    .map((h) => {
      ...
      const body = h.autoGenerated
        ...
        : h.content;
      const note = formatKnowledgeCitationNote(h, staleDays);
