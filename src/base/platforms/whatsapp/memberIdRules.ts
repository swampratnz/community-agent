...
/**
 * WhatsApp's member-id heuristics (agent-base plan item 9): the per-adapter
 * declaration behind `src/auth/memberId.ts`'s `normalizeMemberId('whatsapp',
 * …)` dispatch, plus the LID-resolution helper that is inseparable from the
 * length bound below. Deliberately a leaf module with no Baileys/Cloud/config
 * import — see discord/memberIdRules.ts for the same reasoning.
 */
...
/**
 * A WhatsApp member id is an E.164 number without the `+`: all digits,
 * 7-{@link MAX_WHATSAPP_ID_DIGITS} of them. The error strings are
 * byte-identical to the pre-split `memberId.ts` branches.
 */
export const WHATSAPP_MEMBER_ID_RULES: PlatformMemberIdRules = {
  normalizeMemberId(rawId: string): string {
    const id = rawId.trim().replace(/^\+/, '');
    if (!/^\d+$/.test(id)) {
      throw new Error(
        `"${rawId}" is not a valid whatsapp id: expected digits only (E.164 number without +).`,
      );
    }
    // The two out-of-range cases are deliberately SEPARATE, because they have
    // different causes and so need different advice. Folding them together
    ...
    if (id.length > MAX_WHATSAPP_ID_DIGITS) {
...
export async function resolveWhatsappLid(
  ...
  try {
    return WHATSAPP_MEMBER_ID_RULES.normalizeMemberId(phone);
  } catch {
    return null;
  }
}
