...
import { trustOf } from '../provenance.js';
...
export async function saveKnowledge(input: {
  ...
  // answered" signal. trustOf (storage/provenance.ts) is fail-closed: an
  // unregistered provenance is quarantined, never trusted by accident. See
  // resolveKnowledgeGaps.
  if (embedding && trustOf(createdByRole) === 'trusted') {
    try {
 ...
 *
 * SECURITY: callers gate this on `trustOf(createdByRole) === 'trusted'`
 * (storage/provenance.ts, fail-closed for unregistered values) before invoking
...
export interface KnowledgeSearchHit {
  ...
  similarity: number;
  updatedAt: Date;
  /** True for quarantined-provenance entries (trustOf(created_by_role) — 'auto' machine research, or any UNREGISTERED value, fail-closed) — quarantined at retrieval. */
...
export async function searchKnowledge(
  ...
  return rows.map((r) => ({
    ...
    autoGenerated: trustOf(r.created_by_role) === 'quarantined',
...
export async function listKnowledgeTopics(
  ...
  // `created_by_role != 'auto'` stays as SQL, not trustOf() — the quarantine
  // boundary is evaluated in the database, and the negated form is fail-closed
  // there in a way an IN-list of trusted values would not be. See
  // storage/provenance.ts; tests/provenanceTrust.test.ts pins the `!=` form.
  const { rows } = await pool.query(
...
export async function updateKnowledge(input: {
  ...
  // SECURITY: same fail-closed trust gate as saveKnowledge (trustOf,
  // storage/provenance.ts) — an entry's created_by_role never changes here,
  // so the pre-edit row's value is the authoritative check.
  if (embedding && trustOf(existingRows[0].created_by_role) === 'trusted') {
    try {
      await resolveKnowledgeGaps(scope, embedding, input.callerPlatform ?? null);
    } catch (err) {
...
export async function listCuratedKnowledgeCreatedSince(since: Date, limit: number): Promise<string[]> {
  ...
  // SQL-side quarantine boundary, deliberately not trustOf() — see
  // storage/provenance.ts and the comment on listKnowledgeTopics above.
...
registerPurgeContributor({
  ...
  async purge({ userId }, tx) {
    ...
    const { rowCount: knowledge } = await tx.query(`DELETE FROM knowledge WHERE source_user_id = $1`, [
      userId,
    ]);
    return knowledge ?? 0;
  },
  async summarize({ userId }, db) {
    // knowledge has no platform column (see purge above), so this keys on
    // source_user_id alone, same as the DELETE it reconciles with.
    const { rows: knowledgeRows } = await db.query(
      `SELECT count(*) AS n FROM knowledge WHERE source_user_id = $1`,
      [userId],
    );
    return { knowledgeEntries: Number(knowledgeRows[0]?.n ?? 0) };
  },
});
