...
export const COMMUNITY_COMMANDS: readonly RegisteredCommand[] = [
  ...
  },
  {
    ...
    whatsapp: async (text, msg, role, deps) => {
      ...
      // the full fetched superset's ids — before ranking/slicing — feeding
      // both the demotion inside rankKnowledgeByRetrieval and the caveat
      // below, same gating/fail-safe shape as the tool handler's identical
      // lookup (knowledgeMember.ts) — kept in parity so this zero-model-call
