 ...
 * check) is reused, not reimplemented, here. `lowRatedCaveat` is intentionally
 * never passed (stays `false`, this renderer's audience is an admin acting on
 * freshness/dead-link signals, not the low-rated caveat — out of scope per
 * the proposal). A hit with no source, that is fresh and has never been
 * link-checked (`sourceUnreachable: null`), renders byte-identical to before
 * this change.
 */
export function formatFoundKnowledge(
  ...
  return untrusted(
    ...
    hits
      .map((h) => {
        const note = formatKnowledgeCitationNote(h, staleDays, false, maxAgeDays);
        return (
