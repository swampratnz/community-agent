 ...
 * stays byte-identical.
 *
 * `style` (issue #1448) defaults to `undefined` and, once `lang !== 'mi'`,
 * selects the `notice()` catalogue's `style.plain` variant for the stale/
 * low-rated fragments instead of their `base` English — `notice()` itself
 * already enforces `'mi'` winning over any `style` (see
 * `strings/catalogue.ts`'s `selectNoticeVariant`), so this call site does no
 * precedence logic of its own. With `style` `undefined`/`'standard'`,
 * rendering is byte-identical to before this change.
 */
export function formatKnowledgeCitationNote(
  ...
  style: ResponseStyle | undefined = undefined,
): string {
  const notes: string[] = [];
  ...
  if (
    ...
    notes.push(notice('knowledgeStaleNote', { language: lang, style }));
  }
  if (lowRatedCaveat) {
    notes.push(notice('knowledgeLowRatedCaveat', { language: lang, style }));
  }
 ...
 *
 * `style` (issue #1448) defaults to `undefined`, threaded into the
 * `knowledgeSearchEmpty` early return, `formatKnowledgeCitationNote`'s own
 * `style` parameter, and the trailing conflict caveat — same shape as `lang`
 * above, and independent of it: `knowledge_search`'s own handler resolves a
 * local `lang` only to gate the response-style lookup (never passing it to
 * this function's `lang` parameter, matching that call site's existing
 * `lang`-omission), then passes the resolved `style` alone. With `style`
 * `undefined`/`'standard'`, rendering is byte-identical to before this
 * change.
 */
export function formatKnowledgeSearchResults(
  ...
  if (relevant.length === 0) return notice('knowledgeSearchEmpty', { language: lang, style });
  ...
  const body = ordered
    .map((h) => {
      ...
      const entryBody = h.autoGenerated
        ...
        : h.content;
      const note = formatKnowledgeCitationNote(h, staleDays, lowRatedIds.has(h.id), maxAgeDays, lang, style);
      return `- (${(h.similarity * 100).toFixed(0)}% match) ${h.title ? `${h.title}: ` : ''}${entryBody} (updated ${formatRelativeAge(h.updatedAt)})${note}`;
    })
    .join('\n');
  return hasConflict ? `${body}\n\n(${notice('knowledgeConflictCaveat', { language: lang, style })})` : body;
}
 ...
 *
 * Citation/freshness (issue #1206): `find_knowledge`'s hits already carry the
 * full `KnowledgeCitationInfo` shape (same `searchKnowledge`/
 * `searchKnowledgeLexical` result rows `formatKnowledgeSearchResults` renders
 * a citation note from), but this renderer never called
 * `formatKnowledgeCitationNote` — so the weekly link-rot checker's (#448)
 * dead-link verdict was silently dropped from the one tool an admin uses to
 * locate an entry before mutating it. Each line now appends the same clause
 * `formatKnowledgeSearchResults` would render for the same hit (byte-
 * identical wording), via the shared formatter — so the trust-elevation guard
 * that keeps an `autoGenerated` entry's citation from rendering
 * (`formatKnowledgeCitationNote`'s own `!hit.autoGenerated && hit.sourceUrl`
 * check) is reused, not reimplemented, here. A hit with no source, that is
 * fresh and has never been link-checked (`sourceUnreachable: null`), renders
 * byte-identical to before this change.
 *
 * Low-rated caveat (issue #1336, the growth #1206 deferred): `lowRatedIds`
 * defaults to an empty set, so omitting it leaves output byte-identical to
 * before — the same additive, no-op-when-omitted idiom `formatKnowledgeSearchResults`
 ...
 *
 * `style` (issue #1448) is consulted only once `language !== 'mi'` — same
 * `mi`-wins-over-`plain` precedence every formatter in this file follows —
 * and swaps in a shorter English variant for the same two prose branches.
 * `style` `undefined`/`'standard'` is byte-identical to before this change.
 */
  ...
  const truncationNote =
    remaining > 0
      ? mi
        ...
        : plain
          ? `\n\n+${remaining} more. Ask me a specific question and I'll search everything.`
 ...
 *
 * `style` (issue #1448) defaults to `undefined`, threaded into
 * `formatKnowledgeCitationNote`'s own `style` parameter and the trailing
 * conflict caveat — the empty-KB message above is untouched (it's a fixed
 * `mi`-ternary, not a `notice()` catalogue entry). With `style`
 * `undefined`/`'standard'`, output is byte-identical to before this change.
 */
export function formatMostHelpfulKnowledge(
  ...
  return hasConflict
    ? `${rendered}\n\n(${notice('knowledgeConflictCaveat', { language, style })})`
    : rendered;
}
 ...
 * friendly no-op, not a tool error. `style` (issue #1448), same
 * `mi`-wins-over-`plain` precedence every formatter in this file follows.
 */
export function formatSuggestKnowledgeDedupText(
  language: LanguagePreference,
  ...
  return mi
    ...
    : plain
      ? 'Thanks, but someone already sent a similar tip. No need to send it again.'
 ...
 * languages — only the surrounding prose changes. `style` (issue #1448),
 * same `mi`-wins-over-`plain` precedence as every formatter in this file.
 */
export function formatSuggestKnowledgeRateLimitText(
  limit: number,
  language: LanguagePreference,
  style: ResponseStyle | undefined,
): string {
  ...
  return mi
    ...
    : plain
      ? `You've sent ${limit} tips today. Please wait before sending another.`
 ...
 * functions differing by a single clause. `style` (issue #1448), same
...
export function formatSuggestKnowledgeQueuedText(
  ...
  if (correctionLabel) {
    return mi
      ...
      : plain
        ? `Thanks! This looks like it might update ${correctionLabel}. Tip #${id} is waiting for an admin to check it.`
  ...
  return mi
    ...
    : plain
      ? `Thanks! Tip #${id} is waiting for an admin to check it. It won't be added unless an admin says yes.`
 ...
 * `isError` stays `true` here, unchanged from today. `style` (issue #1448),
 * same `mi`-wins-over-`plain` precedence as every formatter in this file.
 */
export function formatWithdrawKnowledgeTipEmptyText(
  language: LanguagePreference,
  style: ResponseStyle | undefined,
): string {
  return language === 'mi'
 ...
 * same interpolated `#id` list in both languages. `style` (issue #1448),
 * same `mi`-wins-over-`plain` precedence as every formatter in this file.
 */
export function formatWithdrawKnowledgeTipConfirmText(
  ...
  if (style === 'plain') {
    return `Withdrew tip${ids.length > 1 ? 's' : ''} ${list}. No one will review ${ids.length > 1 ? 'them' : 'it'}.`;
  }
  return (
...
/**
 * Pure renderer shared by the `list_access_requests` tool handler and the
 * `!accessrequests`/`/accessrequests` admin shortcut (issue #1346), hoisted
 * verbatim out of the tool handler's own inline rendering so the two call
 * sites can never drift — same reasoning as `formatMutedMembersList`/
 * `formatBlockedMembersList`/`formatTopKnowledgeList`/`formatAdminRoster`
 * above. Empty input renders the tool's own fixed "No pending access
 * requests." string; each row renders through the same `untrusted()`
 * quarantine the tool handler already used (a request carries an
 * admin-visible, possibly hostile `userName`), including the "waiting Nd"
 * age computed from the DB-stored `firstRequestedAt` (never a caller-supplied
 * value — issue #515).
 *
 * `truncated` (default `false`, matching the shortcut's own fixed-default
 * call, which never hits this) reproduces `list_access_requests`'
 * `oldestFirst` truncation caveat verbatim when the tool handler's own
 * bounded scan hits `ACCESS_REQUEST_STALE_ALERT_SCAN_LIMIT` — kept INSIDE the
 * `untrusted()` wrap, matching the tool handler's pre-extraction placement
 * (the caveat text itself is bot-computed and trusted, but the wrap boundary
 * must stay byte-identical to before this hoist).
 */
export function formatAccessRequestsList(rows: readonly AccessRequest[], truncated = false): string {
  if (rows.length === 0) return 'No pending access requests.';
  ...
  return untrusted(
    ...
    rows
      .map((r) => {
        // firstRequestedAt is always the DB-stored insert timestamp for this
        // (platform, user_id) row (repository's listAccessRequests) — never
        // sourced from a tool argument, so it can't be spoofed by a
        // caller-supplied value (issue #515).
