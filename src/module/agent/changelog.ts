 ...
 * production build tsc emits this module to `dist/module/agent/` and the build
 * step copies CHANGELOG.md to `dist/` (mirroring how the schema fragments are bundled), so it
 * sits at `dist/CHANGELOG.md`. Under tsx (dev / tests) this runs from
 * `src/module/agent/` with the file still at the repo root. Try both layouts.
 */
const CANDIDATE_PATHS = [
  join(__dirname, '..', '..', 'CHANGELOG.md'), // dist/module/agent -> dist/CHANGELOG.md (prod)
  join(__dirname, '..', '..', '..', 'CHANGELOG.md'), // src/module/agent -> repo root (dev/tests)
];
