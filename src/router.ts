...
/** The subset of a KnowledgeSearchHit the shortcut path carries through — just enough to render `formatKnowledgeCitationNote` (issue #214) and, since issue #701, `claimStaleKnowledgeAlert`. */
interface KnowledgeShortcutHit {
  id: number;
  /** Only read by the issue #701 stale-alert DM (falls back to a bounded content excerpt when null) — never rendered in the member-facing shortcut reply itself. */
  title: string | null;
  content: string;
  updatedAt: Date;
...
export class Router {
  ...
  constructor(
    ...
    private readonly claimStaleKnowledgeAlertFn: typeof claimStaleKnowledgeAlert = claimStaleKnowledgeAlert,
  ) {
  ...
  /** Rolling-hour timestamps for `KNOWLEDGE_STALE_ALERT_RATE_LIMIT_PER_HOUR` (issue #701) — guild-wide, same reasoning as `knowledgeGapAlertTimestamps` above. */
   ...
   * that method, a miss here does NOT leave anything unstamped to retry —
   * `claimStaleKnowledgeAlertFn` has already stamped `stale_alerted_at`
   * before this is even checked (issue #701 acceptance criterion 5c), so a
   * `false` return here just means the DM itself is skipped this time.
   */
  ...
  /**
   * Fires the real-time stale-knowledge admin DM for an already-claimed
   * entry (issue #701) — `claimStaleKnowledgeAlertFn` has already atomically
   * stamped `stale_alerted_at` by the time this runs, regardless of whether
   * the rate limit below allows the DM to actually send. Shared by all three
   * serve paths: `sendKnowledgeShortcut`/`sendGuestKnowledgeShortcut` below,
   * and the post-turn `reply.staleKnowledgeAlerts` branch in `respond()` for
   * the `knowledge_search` tool path.
   *
   * SECURITY: the DM body names only the entry's title (or a
   * `truncateForEcho`-bounded excerpt) and a relative age — never
   * `excludeUserId`, a query, or a conversation id (acceptance criterion 5a).
   */
  private async fireStaleKnowledgeAlert(
    entry: StaleKnowledgeAlertCandidate,
    excludeUserId: string,
  ): Promise<void> {
    if (!this.reserveStaleKnowledgeAlertSlot(config.knowledgeStaleAlert.rateLimitPerHour)) return;
    const label = entry.title ?? truncateForEcho(entry.content);
    await this.notifyAdminsFn(
      (platform) => this.adapters.get(platform),
      `A knowledge entry may be outdated (last updated ${formatRelativeAge(entry.updatedAt)}) ` +
        `and was just served to a member: "${label}"`,
      excludeUserId,
    ).catch((err) => logger.warn({ err }, 'Stale knowledge admin notification failed'));
  }
  ...
  private async sendKnowledgeShortcut(
    ...
    // same non-blocking placement as the retrieval-count bump above.
    this.claimStaleKnowledgeAlertFn(hit)
      .then((claim) => (claim ? this.fireStaleKnowledgeAlert(claim, msg.userId) : undefined))
      .catch((err) => logger.warn({ err }, 'Stale knowledge admin alert failed'));
  ...
  private async sendGuestKnowledgeShortcut(
    ...
    // Real-time stale-knowledge admin nudge (issue #701) — fire-and-forget,
    // same non-blocking placement as the retrieval-count bump above. Never
    // gated on the guest's own identity (a guest is never an admin), same
    // reachability class as `sendKnowledgeShortcut`'s own alert.
    this.claimStaleKnowledgeAlertFn(hit)
      .then((claim) => (claim ? this.fireStaleKnowledgeAlert(claim, msg.userId) : undefined))
      .catch((err) => logger.warn({ err }, 'Stale knowledge admin alert failed'));
  }
  ...
  private async respond(
    ...
    try {
      ...
      // Real-time admin nudge when a stale knowledge entry is served via the
      // `knowledge_search` tool (issue #701) — the per-entry counterpart to
      // the gap-cluster alert above. `reply.staleKnowledgeAlerts` is only
      // ever set (by the knowledge_search tool handler) when
      // config.knowledgeStaleAlert.enabled is true and the turn ended in
      // genuine success, so no separate flag check is needed here. Every
      // entry listed here was already atomically claimed (`stale_alerted_at`
      // stamped) by `claimStaleKnowledgeAlertFn` inside the turn — the rate
      // limit below only gates whether the DM itself sends, never whether
      // the claim happened, so a miss here can't retry-storm.
      if (reply.staleKnowledgeAlerts) {
        for (const claim of reply.staleKnowledgeAlerts) {
          await this.fireStaleKnowledgeAlert(claim, msg.userId);
        }
      }
