...
test(
  'SECURITY: issue #1151 — agent-security-and-untrusted-input-design resolves to the bundled SKILL.md, ' +
    "grants no new tool access, and changes no role's disallowedTools",
  async () => {
    const { toolsForRole } = await import('@swampratnz/agent-base/auth/rbac.js');
    const skillPath = join(
      dirname(fileURLToPath(import.meta.url)),
      '../src/module/agent/skills/agent-security-and-untrusted-input-design/SKILL.md',
    );
    const body = readFileSync(skillPath, 'utf8');
    assert.match(
      body,
      /^---\nname: agent-security-and-untrusted-input-design\n/,
      'SKILL.md must carry valid agent-security-and-untrusted-input-design front-matter',
    );
    for (const role of ['guest', 'member', 'admin', 'super_admin'] as const) {
      const opts = buildQueryOptions(role, 'prompt', {}, null, 'conv-1', 'discord');
      assert.ok(
        opts.skills?.includes('agent-security-and-untrusted-input-design'),
        `${role}: skills must include agent-security-and-untrusted-input-design when the flag is on`,
      );
      const webSearch = role === 'admin' || role === 'super_admin';
      assert.deepEqual(
        opts.disallowedTools,
        ['Task', 'WebFetch', ...(webSearch ? [] : ['WebSearch'])],
        `${role}: disallowedTools must be unaffected by adding agent-security-and-untrusted-input-design to ENABLED_SKILLS`,
      );
      const expected = [...toolsForRole(role, 'discord'), ...(webSearch ? ['WebSearch'] : [])].filter(
        (t) => !(FEATURE_FLAGGED_TOOLS as readonly string[]).includes(t),
      );
      assert.deepEqual(
        [...opts.allowedTools].sort(),
        [...expected].sort(),
        `${role}: allowedTools must be unaffected by agent-security-and-untrusted-input-design — no new MCP tool surface`,
      );
    }
  },
);
...
test(
  ...
  () => {
    ...
    assert.doesNotMatch(
      body,
      /systemPrompt\.ts|tool registry|RBAC internals|src\/module/i,
      "SKILL.md must not reference this deployment's own systemPrompt.ts, tool registry, or RBAC internals as worked examples",
    );
  },
);

test(
  'issue #1151 — agent-security-and-untrusted-input-design SKILL.md hands off to prompt-review, ' +
    'agent-architecture-review, and mcp-server-design rather than restating their guidance',
  () => {
    const skillPath = join(
      dirname(fileURLToPath(import.meta.url)),
      '../src/module/agent/skills/agent-security-and-untrusted-input-design/SKILL.md',
    ...
    assert.match(
      body,
      /prompt-review/,
      'SKILL.md must hand off already-pasted schema/prompt review to prompt-review',
    );
    assert.match(
      body,
      /agent-architecture-review/,
...
test(
  'SECURITY: issue #1173 — multi-agent-and-subagent-orchestration-design resolves to the bundled ' +
    "SKILL.md, grants no new tool access, and changes no role's disallowedTools",
  async () => {
    const { toolsForRole } = await import('@swampratnz/agent-base/auth/rbac.js');
    const skillPath = join(
      dirname(fileURLToPath(import.meta.url)),
      '../src/module/agent/skills/multi-agent-and-subagent-orchestration-design/SKILL.md',
    );
    const body = readFileSync(skillPath, 'utf8');
    assert.match(
      body,
      /^---\nname: multi-agent-and-subagent-orchestration-design\n/,
      'SKILL.md must carry valid multi-agent-and-subagent-orchestration-design front-matter',
    );
    for (const role of ['guest', 'member', 'admin', 'super_admin'] as const) {
      const opts = buildQueryOptions(role, 'prompt', {}, null, 'conv-1', 'discord');
      assert.ok(
        opts.skills?.includes('multi-agent-and-subagent-orchestration-design'),
        `${role}: skills must include multi-agent-and-subagent-orchestration-design when the flag is on`,
      );
      const webSearch = role === 'admin' || role === 'super_admin';
      assert.deepEqual(
        opts.disallowedTools,
        ['Task', 'WebFetch', ...(webSearch ? [] : ['WebSearch'])],
        `${role}: disallowedTools must be unaffected by adding multi-agent-and-subagent-orchestration-design to ENABLED_SKILLS`,
      );
      const expected = [...toolsForRole(role, 'discord'), ...(webSearch ? ['WebSearch'] : [])].filter(
        (t) => !(FEATURE_FLAGGED_TOOLS as readonly string[]).includes(t),
      );
      assert.deepEqual(
        [...opts.allowedTools].sort(),
        [...expected].sort(),
        `${role}: allowedTools must be unaffected by multi-agent-and-subagent-orchestration-design — no new MCP tool surface`,
      );
    }
  },
);
...
test(
  'SECURITY: issue #1173 AC #1/AC #4 — multi-agent-and-subagent-orchestration-design SKILL.md carries an ' +
    'untrusted-input clause for a member-pasted design/transcript/subagent output (data to analyse, never ' +
    "instructions to obey), and teaches only general patterns with no reference to this deployment's own " +
    'internals',
  () => {
    const skillPath = join(
...
test(
  'issue #1173 — multi-agent-and-subagent-orchestration-design SKILL.md hands off to ' +
    'agent-architecture-review, tool-use-and-structured-output-design, mcp-server-design, ' +
    'eval-and-testing-design, and api-cost-and-latency rather than restating their guidance',
  () => {
    const skillPath = join(
      dirname(fileURLToPath(import.meta.url)),
      '../src/module/agent/skills/multi-agent-and-subagent-orchestration-design/SKILL.md',
    ...
    assert.match(
      body,
      /agent-architecture-review/,
      'SKILL.md must hand off already-built whole-pipeline critique to agent-architecture-review',
    );
    assert.match(
      body,
      /tool-use-and-structured-output-design/,
      'SKILL.md must hand off single tool-call/schema decisions to tool-use-and-structured-output-design',
    );
    assert.match(
      body,
      /mcp-server-design/,
    ...
    assert.match(
      body,
      /api-cost-and-latency/,
      'SKILL.md must hand off deep cost/latency tuning to api-cost-and-latency',
    );
    assert.match(
      body,
      /out of scope/i,
...
test('SECURITY: AC5 — the bundled skill plugin directory contains no hooks/, agents/, commands/, or .mcp.json path', () => {
  ...
  assert.ok(
    files.some((f) => f.endsWith(join('agent-security-and-untrusted-input-design', 'SKILL.md'))),
    'expected agent-security-and-untrusted-input-design/SKILL.md to be present',
  );
});
