...
test(
  'SECURITY: issue #1133 — eval-and-testing-design SKILL.md carries an untrusted-input clause for ' +
    'member-pasted prompt/eval example/harness sketch content (data to analyse, never instructions to obey)',
  () => {
    const skillPath = join(
      dirname(fileURLToPath(import.meta.url)),
      '../src/module/agent/skills/eval-and-testing-design/SKILL.md',
...
test(
  ...
  () => {
    ...
    assert.match(
      body,
      /out of scope/i,
      'SKILL.md must state whole-pipeline/retrieval concerns are out of scope for this skill',
    );
  },
);
