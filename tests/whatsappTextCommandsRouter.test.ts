...
// --- !kbhelpful low-rated caveat parity (issue #1143) -------------------------
...
test('SECURITY: !kbhelpful still replies successfully with the entries and no caveat when areKnowledgeEntriesLowRated rejects (fail-safe, issue #1143 acceptance criterion 4)', async (t) => {
  ...
  assert.equal(sent.length, 1);
  assert.ok(sent[0].text.includes('STILL_SERVED_KBHELPFUL_TEXT'), 'the entry must still be served');
  assert.ok(
    !sent[0].text.includes(KNOWLEDGE_LOW_RATED_CAVEAT_TEXT),
...
/**
 * Stubs `pool.query`'s role branch plus the five `review_queue`-shortcut
 * repository reads (access requests, suggestions, knowledge candidates,
 * reports, appeals) — each table's count/age pair is distinguished by
...
test(
  ...
  async (t) => {
    ...
    assert.doesNotMatch(
      sent[0].text,
      /Reports/,
      'the reports line must be omitted entirely, never fabricated',
    );
  },
);
