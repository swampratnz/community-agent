...
test('SECURITY: list_blocked_members is admin-tier only — absent from the member/guest surface (issue #924)', () => {
  const tool = 'mcp__community__list_blocked_members';
  assert.ok(toolsForRole('admin').includes(tool), 'admins have list_blocked_members');
  assert.ok(toolsForRole('super_admin').includes(tool), 'super admins have list_blocked_members');
  assert.ok(!toolsForRole('member').includes(tool), 'members must not have list_blocked_members');
  assert.ok(!toolsForRole('guest').includes(tool), 'guests must not have list_blocked_members');
});
