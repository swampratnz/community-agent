...
test('SECURITY: AC-4 — a thrown error inside the WebSearch dedup check fails closed (denies), never lets the call through unbounded', async (t) => {
  ...
  // tests/agentWebSearchRateLimitFailClosed.test.ts). Preserve every real
  // export except isDuplicateWebSearchQuery, which is replaced with one that
  ...
  // exception.
  const real = await import('@swampratnz/agent-base/agent/webSearchGuard.js');
  t.mock.module('@swampratnz/agent-base/agent/webSearchGuard.js', {
    namedExports: {
      ...real,
      isDuplicateWebSearchQuery: () => {
        throw new Error('boom: simulated dedup-check failure');
      },
    },
  });
  ...
  assert.equal(
    ...
    'deny',
    'a thrown dedup check must fail closed (deny), never allow the call through',
  );
});
