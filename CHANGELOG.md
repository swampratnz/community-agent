...
- **`my_data` (and its `/mydata`/`!mydata` mirrors) now reports your filed
  ...
  sent:` lines alongside the existing counts.
...
- **New Agent Skill: `eval-and-testing-design`, for members designing tests
  or evals for their own Claude-powered feature.** (#1133) "How do I know if
  my prompt change actually made things better?", "how big does my golden
  set need to be?", "LLM-as-judge or exact-match?" were a distinct, common
  builder question none of the twelve existing skills covered —
  `agent-architecture-review` treats evaluation as one checklist bullet
  among five, `rag-and-retrieval-design` hands "evaluation-beyond-retrieval"
  back out, and `prompt-review` never asks the regression question. The new
  skill branches first on single-prompt output quality vs. whole-agent
  behaviour (handing the latter to `agent-architecture-review`), then walks
  golden-set sizing/composition, grading method (exact-match vs.
  rubric-graded vs. LLM-as-judge, including the self-grading-bias tradeoff),
  gate-before-ship discipline, and statistical caution as a branch-by-lever
...
- **`my_warnings` and `my_submissions` (and their `/warnings`/`!warnings`,
  ...
  these two self-service tools always answered in English regardless of the
  caller's stored preference, unlike `/kb`, `/digest`, `community_info` and
  the guidelines/moderation DMs, which already do this. A member checking
...
- **The bot now remembers which WhatsApp privacy id belongs to which number**
  (#935). WhatsApp gives people a phone number and a separate "privacy id", and only the
...
- **Members added by a WhatsApp "LID" are now refused instead of silently
  failing** (#934). WhatsApp gives each person a phone number *and* a privacy id
...
- **New `agent-architecture-review` Agent Skill** (#755, same
  `AGENT_SKILLS_ENABLED` flag as #741, off by default): a second on-demand
  skill covering a distinct member ask from `prompt-review` — a critique of a
  member's *multi-step agent or pipeline design* (stage/model fit, tool
  surface & least-privilege, evaluation/verification, cost & latency, failure
  handling) rather than a single pasted prompt. Added to the same
  hand-written, code-reviewed `ENABLED_SKILLS` allowlist as `prompt-review`
  (never `'all'`); grounds recommendations in `knowledge_search` with the
